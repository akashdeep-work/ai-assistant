...
┌─────────────────┐       1. Submit Prompt        ┌─────────────────┐
│                 │ ────────────────────────────► │  Apache Kafka   │
│  FastAPI Server │                               │  (Broker, 29092)│
│  (Port 8000)    │ ◄──────────────────────────── │                 │
└────────┬────────┘       4. Token Streams        └────────┬────────┘
         │               (via Redis Pub/Sub)               │
         │                                                 │ 2. Consume Task
         │                                                 ▼
         │   Read History   ┌─────────────────┐   Process  ┌─────────────────┐
         └────────────────► │   Shared DB     │ ◄───────── │   AI Worker     │
                            │ (SQLite Volume) │            │ (worker.py)     │
                            └─────────────────┘            └────────┬────────┘
                                                                    │
                                                                    │ 3. Run RAG/LLM
                                                                    ▼
...
⚠️ Note: Keep the configuration key KAFKA_BOOTSTARP_SERVER spelled exactly as shown to match the internal configuration mapper.
...
[ Git Push to Main ] ──► [ Pipeline Runs Mocks ] ──► [ Pass? ] ──► [ Auto-Deploy via SSH ]
Every time you execute a code update to your main branch, the pipeline will automatically:

Construct a temporary virtual machine in the cloud.

Inject your production environment layout variables.

Boot isolated Redis runtime layers.

Execute your test collection using TEST_MODE=mock.

Only if all tests pass: Connect safely to your live Linux deployment server over SSH, pull down the code changes, and execute a zero-downtime rolling restart (docker-compose up --build -d).
